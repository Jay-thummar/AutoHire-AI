...
Create Job → Paste JD text
AI parses JD → Extracts skills, experience, requirements
Generate Assessment → AI creates MCQ + subjective + coding questions
Toggle Anonymous Mode (optional)
Monitor live sessions via WebSocket dashboard
View results → Leaderboard, rankings, proctoring reports
Download AI-generated candidate analytics reports
Reveal candidate identities (if anonymous mode was on)
Candidate Flow
Register/Login → Browse jobs with AI recommendations
Click Apply → Redirected to resume upload (mandatory gate)
Upload PDF resume → Three-layer screening runs
If approved → Access assessment; if rejected → Detailed feedback with reasons
Take timed assessment under AI proctoring (webcam + DOM monitoring)
Submit → AI evaluates all answers → Fraud detection runs
View results → Score, rank, percentile, detailed report
Setup
Prerequisites
Node.js 18+
